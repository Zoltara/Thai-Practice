...
import { generatePracticeWord, evaluatePronunciation, generateSpeech, generateSpeechStream } from '../services/geminiService';
import { blobToBase64, decode, decodeAudioData } from '../utils/audio';

interface ThaiSpeakingSessionProps {
  topic: string;
  onNext: () => void;
...
const ThaiSpeakingSession: React.FC<ThaiSpeakingSessionProps> = ({
  topic,
  onNext,
  progressCount,
  totalItems,
  ...
  const handleStartRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        setIsProcessing(true);
        try {
          const result = await evaluatePronunciation(currentTarget!.word, base64, 'audio/webm', 'Thai');
          setFeedback(result);
        } catch (e) {
        ...
        {feedback && (
          <div className="bg-slate-900/50 rounded-2xl p-6 border border-slate-700 animate-fade-in-up">
            ...
            <div className="flex gap-3 mt-6">
              <button 
                onClick={() => onToggleDictionaryWord({ word: currentTarget!.word, english: currentTarget!.english, phonetic: currentTarget!.phonetic })} 
                className={`flex-1 py-4 rounded-xl font-black uppercase tracking-widest transition-all text-sm ${isSaved ? 'bg-amber-500 text-slate-900' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
              >
...
export default ThaiSpeakingSession;
